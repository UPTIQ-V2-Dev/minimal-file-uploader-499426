...
import { FileUpload, FileUploadStatus } from '../generated/prisma/index.js';
 ...
 */
const validateFile = (fileName: string, fileType: string, fileSize: number) => {
 ...
 * @returns {string}
 */
const generateFileKey = (fileName: string, userId: number): string => {
 ...
 */
const generateSignedUrl = async (fileKey: string, fileType: string): Promise<string> => {
...
const initiateUpload = async (
    ...
    fileSize: number,
    userId: number
): Promise<{ uploadId: string; signedUrl: string; fileUrl: string }> => {
    // Validate file parameters
 ...
 */
const completeUpload = async (uploadId: string, userId: number): Promise<{ uploadId: string; fileUrl: string }> => {
 ...
 */
const getUploadById = async (uploadId: string, userId: number): Promise<FileUpload | null> => {
    const upload = await prisma.fileUpload.findUnique({
        where: { uploadId }
    });
 ...
 * @returns {Promise<FileUpload>}
 */
const deleteUpload = async (uploadId: string, userId: number): Promise<FileUpload> => {
