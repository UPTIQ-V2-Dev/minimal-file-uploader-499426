 ...
 */
const executeOperation = async (userId: number, operation: string, data?: any): Promise<MCPOperation> => {
    ...
    try {
        ...
        let result: any;
        let status: MCPOperationStatus = MCPOperationStatus.COMPLETED;
    ...
    try {
        switch (type) {
            ...
            case 'resources':
                ...
                const resourceFilter: any = { userId };
                if (resourceType) resourceFilter.type = resourceType;
                if (status) resourceFilter.status = status;
                if (filter) {
                    resourceFilter.OR = [
            ...
            case 'operations':
                ...
                const operationFilter: any = { userId };
                if (filter) {
 ...
 * @returns {Promise<MCPResource>}
 */
const getMCPResourceById = async (userId: number, resourceId: string) => {
    const resource = await prisma.mCPResource.findFirst({
        where: { id: resourceId, userId },
        include: {
            operations: {
 ...
 * @returns {Promise<MCPResource>}
 */
const updateMCPResource = async (userId: number, resourceId: string, updateData: Prisma.MCPResourceUpdateInput) => {
    // Verify resource exists and belongs to user
 ...
 * @returns {Promise<MCPOperation>}
 */
const getMCPOperationById = async (userId: number, operationId: string): Promise<MCPOperation> => {
    const operation = await prisma.mCPOperation.findFirst({
