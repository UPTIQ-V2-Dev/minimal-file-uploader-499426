...
describe('MCP Service', () => {
    ...
    describe('getMCPResourceById', () => {
        ...
        it('should throw error for non-existent resource', async () => {
            await expect(mcpService.getMCPResourceById(testUser.id, 'non-existent-id')).rejects.toThrow(
                'Resource not found'
            );
        });
        ...
        it("should throw error when accessing other user's resource", async () => {
            ...
            await expect(mcpService.getMCPResourceById(adminUser.id, createdResource.id)).rejects.toThrow(
                'Resource not found'
            );
        });
    });
    ...
    describe('updateMCPResource', () => {
        ...
        it('should throw error when updating non-existent resource', async () => {
            await expect(
                mcpService.updateMCPResource(testUser.id, 'non-existent-id', { name: 'New Name' })
    ...
    describe('deleteMCPResource', () => {
        ...
        it('should delete operation successfully', async () => {
            ...
            await expect(mcpService.getMCPOperationById(testUser.id, operation.id)).rejects.toThrow(
                'Operation not found'
            );
        });
        ...
        it('should throw error for invalid delete type', async () => {
            await expect(mcpService.deleteMCPResource(testUser.id, 'some-id', 'invalid_type')).rejects.toThrow(
                'Invalid delete type'
            );
        });
    });
    ...
    describe('getMCPData', () => {
        ...
        it('should return status data', async () => {
            ...
            expect((statusData.data[0] as any).activeResources).toBeGreaterThan(0);
        });
        ...
        it('should filter resources by type', async () => {
            ...
            processResources.data.forEach((resource: any) => {
        ...
        it('should filter resources by text search', async () => {
            ...
            filteredResources.data.forEach((resource: any) => {
                expect(resource.name.toLowerCase()).toContain('active');
            });
        });

        it('should throw error for invalid type', async () => {
            await expect(mcpService.getMCPData(testUser.id, { type: 'invalid_type' })).rejects.toThrow(
    ...
    describe('getMCPOperationById', () => {
        ...
        it('should throw error for non-existent operation', async () => {
            await expect(mcpService.getMCPOperationById(testUser.id, 'non-existent-id')).rejects.toThrow(
                'Operation not found'
            );
        });
        ...
        it("should throw error when accessing other user's operation", async () => {
            ...
            await expect(mcpService.getMCPOperationById(adminUser.id, createdOperation.id)).rejects.toThrow(
                'Operation not found'
            );
        });
    });
