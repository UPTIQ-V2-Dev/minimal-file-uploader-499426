...
describe('FileUpload Service', () => {
    ...
    describe('validateFile', () => {
        ...
        it('should reject invalid file types', () => {
            ...
            invalidTypes.forEach(fileType => {
                expect(() => fileUploadService.validateFile('test.file', fileType, 1024)).toThrow(
        ...
        it('should reject files that are too large', () => {
            ...
            expect(() => fileUploadService.validateFile('test.pdf', 'application/pdf', largeFileSize)).toThrow(
                'File too large'
            );
        });
    ...
    describe('initiateUpload', () => {
        ...
        it('should initiate upload successfully', async () => {
            const result = await fileUploadService.initiateUpload(
                validUploadData.fileName,
                validUploadData.fileType,
                validUploadData.fileSize,
                testUser.id
            );
        ...
        it('should reject invalid file parameters', async () => {
            await expect(
                fileUploadService.initiateUpload('test.exe', 'application/exe', 1024, testUser.id)
            ).rejects.toThrow('Unsupported file type');

            await expect(
                fileUploadService.initiateUpload('test.pdf', 'application/pdf', 11 * 1024 * 1024, testUser.id)
    ...
    describe('completeUpload', () => {
        ...
        it('should throw error for non-existent upload', async () => {
            ...
            await expect(fileUploadService.completeUpload(fakeUploadId, testUser.id)).rejects.toThrow(
                'Upload not found'
            );
        });
        ...
        it('should throw error if upload is already completed', async () => {
            ...
            await expect(fileUploadService.completeUpload(uploadId, testUser.id)).rejects.toThrow(
    ...
    describe('queryUploads', () => {
        ...
        it('should filter uploads by status', async () => {
            const result = await fileUploadService.queryUploads(
        ...
        it('should sort uploads by file size', async () => {
            const result = await fileUploadService.queryUploads(
                {},
                { page: 1, limit: 10, sortBy: 'fileSize', sortType: 'asc' },
                testUser.id
            );
