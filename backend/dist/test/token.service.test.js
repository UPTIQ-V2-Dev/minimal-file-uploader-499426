...
describe('Token Service', () => {
    ...
    describe('saveToken', () => {
        it('should save token to database', async () => {
            ...
            const savedToken = await tokenService.saveToken(tokenString, testData.testUser.id, expires, TokenType.REFRESH);
        ...
        it('should save token as blacklisted when specified', async () => {
            ...
            const savedToken = await tokenService.saveToken(tokenString, testData.testUser.id, expires, TokenType.ACCESS, true);
    ...
    describe('generateAuthTokens', () => {
        it('should generate both access and refresh tokens', async () => {
            ...
            const accessDecoded = jwt.verify(tokens.access.token, config.jwt.secret);
            const refreshDecoded = jwt.verify(tokens.refresh.token, config.jwt.secret);
        ...
        it('should generate tokens with correct expiration times', async () => {
            ...
            expect(accessActual.isBetween(accessExpected.subtract(1, 'second'), afterGeneration.add(config.jwt.accessExpirationMinutes, 'minutes'))).toBe(true);
            ...
            expect(refreshActual.isBetween(refreshExpected.subtract(1, 'second'), afterGeneration.add(config.jwt.refreshExpirationDays, 'days'))).toBe(true);
        });
    });
    describe('generateResetPasswordToken', () => {
        ...
        it('should throw error for non-existent user', async () => {
            await expect(tokenService.generateResetPasswordToken('nonexistent@example.com')).rejects.toThrow('No users found with this email');
        });
        it('should generate token with correct expiration time', async () => {
            ...
            expect(tokenExpiration.isBetween(expectedExpiration.subtract(1, 'second'), afterGeneration.add(config.jwt.resetPasswordExpirationMinutes, 'minutes'))).toBe(true);
        });
    });
    describe('generateVerifyEmailToken', () => {
        ...
        it('should generate token with correct expiration time', async () => {
            ...
            expect(tokenExpiration.isBetween(expectedExpiration.subtract(1, 'second'), afterGeneration.add(config.jwt.verifyEmailExpirationMinutes, 'minutes'))).toBe(true);
        });
    });
});
